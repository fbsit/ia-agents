...
def build_hybrid_agent_graph(toolset: AgentToolset, rag_intents: set[str] | None = None, confidence_threshold: float = 0.45):
    active_rag_intents = rag_intents or set()

    def normalize_node(state: HybridAgentState) -> HybridAgentState:
        return {"cleaned_text": normalize_text(state.get("raw_text", ""))}
    ...
    def route_node(state: HybridAgentState) -> HybridAgentState:
        ...
        if toolset.intent_router is not None:
            should_use_rag = (label in active_rag_intents and confidence >= confidence_threshold) if active_rag_intents else confidence >= confidence_threshold
    ...
    def rag_node(state: HybridAgentState) -> HybridAgentState:
        answer = toolset.answer_company_question(
            query=state.get("rag_query", state.get("raw_text", "")),
        ...
        return {"final_answer": answer.answer, "sources": answer.sources}

    def fallback_node(state: HybridAgentState) -> HybridAgentState:
        return {
            "final_answer": "No tengo suficiente confianza para responder automaticamente. Te recomiendo pedir una aclaracion o derivar a un humano.",
            "sources": [],
        }

    def route_selector(state: HybridAgentState) -> str:
        return "rag" if state.get("route", "fallback") == "rag" else "fallback"
    ...
    graph.add_conditional_edges("route", route_selector, {"rag": "rag", "fallback": "fallback"})
