...
class IntentRouter:
    def __init__(self, model_path: str = "models/intent_router.joblib") -> None:
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"No existe modelo de intent router en {path}. "
                "Entrena uno y vuelve a intentar."
            )
    ...
    def _decision_values(self, clean_text: str) -> Any:
        ...
        raise ValueError(
            "El modelo no expone decision_function ni predict_proba para estimar confianza."
        )
    ...
    def predict(self, text: str) -> IntentPrediction:
        ...
        score_label, confidence, scores = build_scores_from_decision(
            self.labels,
            decision_values,
        )
