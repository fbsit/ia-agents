...
def mysql_config_from_env() -> MySQLConfig:
    ...
    return MySQLConfig(
        ...
        query=os.getenv(
            "MYSQL_QUERY",
            "SELECT text, label FROM tweet WHERE label <> '' LIMIT 5000;",
        ),
    )
...
def load_mysql_dataset(config: MySQLConfig) -> pd.DataFrame:
    try:
        ...
        raise ImportError(
            "mysql-connector-python no esta instalado. Corre pip install -r requirements.txt"
        ) from exc
    ...
    if missing:
        raise ValueError(
            "La query debe devolver columnas text y label. "
            f"Faltan: {missing}"
        )
    return df[["text", "label"]].dropna()
