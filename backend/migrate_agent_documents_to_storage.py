...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Migra documentos locales de agentes a backend de storage configurable"
    )
    parser.add_argument(
        "--db-path",
        default=os.getenv("SQLITE_DB_PATH", "data/local_api.db"),
        help="Ruta de SQLite con tablas agents/agent_documents",
    )
    parser.add_argument(
        "--knowledge-root",
        default=os.getenv("AGENTS_KNOWLEDGE_ROOT", "knowledge_base/agents"),
        help="Raiz local de documentos legados",
    )
    parser.add_argument(
        "--backend",
        choices=["local", "s3"],
        default=os.getenv("DOC_STORAGE_BACKEND", "local"),
        help="Backend destino para guardar documentos",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Solo simula migracion sin actualizar la base",
    )
    parser.add_argument(
        "--delete-source",
        action="store_true",
        help="Borra archivos fuente luego de migrar (solo si backend destino no es local)",
    )
...
def _build_agent(row: sqlite3.Row) -> AgentRecord:
    return AgentRecord(
        ...
        created_at=datetime.fromisoformat(row["agent_created_at"]),
        updated_at=datetime.fromisoformat(row["agent_updated_at"]),
    ...
    with sqlite3.connect(args.db_path, timeout=30) as conn:
        ...
        rows = conn.execute(
            ...
            SELECT
                ...
                a.description,
                a.rag_backend,
                a.generation_provider,
                a.use_openai_generation,
                a.openai_model,
                a.knowledge_dir,
                a.index_path,
                a.created_at AS agent_created_at,
                a.updated_at AS agent_updated_at,
                a.indexed_at AS agent_indexed_at
        ...
        for row in rows:
            ...
            try:
                ...
                failed += 1
                print(
                    f"[ERROR] document_id={document.document_id} filename={document.filename} detail={exc}"
                )
