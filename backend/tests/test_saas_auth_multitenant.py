...
def test_long_inactivity_resets_without_confirmation_prompt(monkeypatch: pytest.MonkeyPatch) -> None:
    module, _client = _load_api(monkeypatch, compat_mode="false")
...
def test_product_lookup_followup_uses_persisted_selected_product_when_recent_cache_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    module, _client = _load_api(monkeypatch, compat_mode="false")
    ...
    payload = module._resolve_shared_commerce_payload(  # type: ignore[attr-defined]
        ...
        agent_id="demo-agent",
        user_id="user-1",
        session_id="session-persisted-milo",
        message="Agregalo al carrito",
        channel="whatsapp",
        clubhx_tools_client=FakeToolsClient(),
        intent_label=None,
        response_style_context="",
        workflow_state={"stage": "product_lookup", "pending_next_step": "add_to_cart", "selected_products": "Milo"},
    )
...
def test_direct_cart_message_quiero_2_milo_builds_cart_without_llm(monkeypatch: pytest.MonkeyPatch) -> None:
    module, _client = _load_api(monkeypatch, compat_mode="false")
    ...
    payload = module._resolve_shared_commerce_payload(  # type: ignore[attr-defined]
        ...
        agent_id="demo-agent",
        user_id="user-1",
        session_id="session-direct-cart",
        message="Osea quiero 2 milo",
        channel="whatsapp",
        clubhx_tools_client=FakeToolsClient(),
        intent_label=None,
        response_style_context="",
...
def test_quantity_only_followup_si_2_uses_focused_product(monkeypatch: pytest.MonkeyPatch) -> None:
    module, _client = _load_api(monkeypatch, compat_mode="false")
...
def test_named_followup_el_milo_uses_quantity_or_action_slot(monkeypatch: pytest.MonkeyPatch) -> None:
    module, _client = _load_api(monkeypatch, compat_mode="false")
