...
**El flujo commerce corre antes del RAG.** `AgentService.chat()` llama a `_resolve_commerce_answer()`, que construye el cliente ClubHx por agente (`agents/commerce/tenant_config.py`: primero `agent.clubhx_tenant_id` / `agent.clubhx_shop_domain` guardados en la BD y editables por API y consola; `CLUBHX_TENANT_MAP` y `CLUBHX_TENANT_ID` son solo fallback de entorno) y ejecuta `routing.resolve_shared_commerce_payload()` con los resolvers inyectados; si devuelve payload (búsqueda, carrito, OTP, despacho, pago, orden) esa es la respuesta y se persiste el estado; si no, sigue el RAG. Sin ClubHx configurado para la empresa el paso se omite. El contrato real de tools se consulta en `POST /ai/tools/contracts` de ClubHx; hoy son 10 (`get_product_availability`, `get_shipping_options`, `get_payment_options`, `send_verification_code`, `verify_verification_code`, `login_with_code`, `create_address`, `create_order_draft`, `create_payment_link`, `get_order_status`). La autenticación del cliente es solo por OTP verificado por ClubHx; ninguna frase del usuario autentica.
